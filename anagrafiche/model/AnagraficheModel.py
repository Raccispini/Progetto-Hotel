...
class AnagraficheModel():
    ...
    def add_cliente(self, cliente):
        ...
        data = (cliente.get_nome(), cliente.get_cognome(), cliente.get_sesso(), cliente.get_data_di_nascita().toString("dd/MM/yyyy"),
                ...
                cliente.get_modalita_pagamento(), cliente.get_info_checkin())

        self.db.execute(query, data)
        self.db.commit()

    def add_dipendente(self, dipendente):
        query = "INSERT INTO Dipendenti(username, password, nome, cognome, email, cellulare, data_nascita, "\
                "ambito, permessi) VALUES (?,?,?,?,?,?,?,?,?);"

        data = (dipendente.get_username(), dipendente.get_password(), dipendente.get_nome(), dipendente.get_cognome(),
                dipendente.get_email(), dipendente.get_cellulare(), dipendente.get_data_di_nascita().toString("dd/MM/yyyy"),
                dipendente.get_ambito(), dipendente.get_permessi())

        self.db.execute(query, data)
        self.db.commit()

    def add_fornitore(self, fornitore):
        query = "INSERT INTO Fornitori(nome_fornitore, tipo_fornitura1, tipo_fornitura2, riferimento, "\
                "cellulare_riferimento, email, indirizzo, IVA, modalita_pagamento, telefono, fax) VALUES (?,?,?,?,?,?,?,?,?,?,?);"
